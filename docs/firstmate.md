...
The `Firstmate` group appears after question and error groups and before `Sessions`. It shows canonical durable queued, in-flight, held, and blocked work across OpenCode, Codex, and other harnesses. The inventory does not depend on a live OpenCode session.

Rows include the canonical task state and available project, harness, backend, worktree, hold, blocker, provenance, and freshness details. Selecting a row opens read-only details. There is currently no session navigation because `fm-fleet-snapshot.v1` has no durable OpenCode session mapping. The normalizer ignores undeclared extension fields rather than treating them as navigation data.
