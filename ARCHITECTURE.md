...
| `realtimeBlock.ts` | static definition of the OES realtime widget: tabs, selector rows (each with a `unit`), series readers | `StatRealtimeBlock`, `StatRealtimeTab`, `StatRealtimeRowTab`, `StatRealtimeTabId`, `StatRealtimeSeriesKey`, `seriesValues()`, `realtimeTab()`, `realtimeRow()`, `STAT_REALTIME_BLOCK` |
